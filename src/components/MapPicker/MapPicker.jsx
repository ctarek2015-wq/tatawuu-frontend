...
const MapPicker = ({ latitude, longitude, onChange }) => {
  ...
  return <fieldset>
    ...
    {selected ? <>
      <p>{t("Selected location")}: <bdi dir="ltr">{latitude.toFixed(5)}, {longitude.toFixed(5)}</bdi></p>
      <button type="button" onClick={() => onChange({ latitude: null, longitude: null })}>{t("Remove pin")}</button>
