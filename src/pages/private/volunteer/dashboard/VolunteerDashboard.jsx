...
const VolunteerDashboard = () => {
  ...
  return <main>
    <h1>My activities</h1>
    <button type="button" disabled={tab === "upcoming"} onClick={() => setTab("upcoming")}>Upcoming / in progress</button>
    <button type="button" disabled={tab === "past"} onClick={() => setTab("past")}>Past</button>
    {loading && <p>Loading activities...</p>}
    {error && <p role="alert">{error}</p>}
    {!loading && !error && visibleCampaigns.length === 0 && <p>No {tab} activities.</p>}
    {visibleCampaigns.map((campaign) => {
      ...
      return <article key={campaign._id}>
        ...
        <p>{campaign.venue}, {campaign.address}, {campaign.area}, {campaign.governorate}</p>
        <p>Campaign: {campaign.status}</p>
        <p>Registration: {participant?.status} · Attendance: {participant?.attendance}</p>
        {isPublic && <p><Link to={`/campaigns/${campaign._id}`}>View activity</Link></p>}
        {campaign.status === "Cancelled" && <p>This activity will not take place.</p>}
        {participant?.status === "Registered" && new Date(campaign.startsAt) > new Date() && <button type="button" disabled={busy !== ""} onClick={() => handleCancel(campaign._id)}>
          {busy === campaign._id ? "Cancelling..." : "Cancel registration"}
        </button>}
        {campaign.certificates.includes(user._id) && <p><Link to="/my/certificates">View my certificate</Link></p>}
      </article>;
    })}
  </main>;
};
